...
const navItems = [
  ...
  { href: "/admin/winners", label: "Winners", icon: Star },
]
...
export function AdminNav() {
  ...
  const { data: user } = useSWR("admin-user", async () => {
  ...
  const initials = user?.user_metadata?.first_name?.[0] ?? user?.email?.[0]?.toUpperCase() ?? "A"
            ...
            <DropdownMenuTrigger asChild>
              <button className="flex w-full items-center gap-3 rounded-lg p-2 text-left transition-colors hover:bg-muted">
                <Avatar className="h-9 w-9">
                  <AvatarFallback className="bg-destructive/10 text-destructive">{initials}</AvatarFallback>
                </Avatar>
